...
def apply_multiple_pricing_rules(pricing_rules):
    ...
    return not not apply_multiple_rule
...
def get_other_conditions(conditions, values, args):
    ...
    if args.get("doctype") in {"Quotation", "Quotation Item", "Sales Order", "Sales Order Item", "Delivery Note", "Delivery Note Item", "Sales Invoice", "Sales Invoice Item", "POS Invoice", "POS Invoice Item"}:
...
def filter_pricing_rules(args, pricing_rules, doc=None):
    ...
    if len(pricing_rules) > 1:
        filtered_rules = [x for x in pricing_rules if x.currency == args.get("currency")]
        if filtered_rules:
    ...
    if pricing_rules:
        ...
        if max_priority:
            pricing_rules = [x for x in pricing_rules if cint(x.priority) == max_priority]
    ...
    if len(pricing_rules) > 1:
        ...
        if len(rate_or_discount) == 1 and rate_or_discount[0] == "Discount Percentage":
            pricing_rules = (
                [x for x in pricing_rules if x.for_price_list == args.price_list] or pricing_rules
            )
...
def filter_pricing_rules_for_qty_amount(qty, rate, pricing_rules, args=None):
    ...
    for rule in pricing_rules:
        ...
        status = flt(rate) >= flt(rule.min_amt) * conversion_factor and (flt(rate) <= rule.max_amt * conversion_factor if rule.max_amt else True) and status
...
def apply_internal_priority(pricing_rules, field_set, args):
    ...
    for field in field_set:
        if args.get(field):
            ...
            filtered_rules = [x for x in pricing_rules if x.get(field) == args.get(field)]
            if filtered_rules:
                break
