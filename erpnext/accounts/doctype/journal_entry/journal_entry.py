...
class JournalEntry(AccountsController):
    ...
    def update_advance_paid(self):
        ...
        for voucher_type, order_list in advance_paid.items():
            for voucher_no in set(order_list):
    ...
    def update_invoice_discounting(self):
        ...
        invoice_discounting_list = list(
            {d.reference_name for d in self.accounts if d.reference_type == "Invoice Discounting"}
        )
    ...
    def check_credit_limit(self):
        customers = list(
            {d.party for d in self.get("accounts") if d.party_type == "Customer" and d.party and (flt(d.debit) > 0)}
        )
        if customers:
    ...
    def validate_reference_doc(self):
        ...
        for d in self.get("accounts"):
            ...
            if d.reference_type and d.reference_name and (d.reference_type in field_dict):
                dr_or_cr = (
                    "credit_in_account_currency"
    ...
    def get_values(self):
        ...
        if self.write_off_based_on == "Accounts Payable":
            return frappe.db.sql(
