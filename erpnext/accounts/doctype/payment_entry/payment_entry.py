...
class PaymentEntry(AccountsController):
    ...
    def validate_allocated_amount_with_latest_data(self):
        if self.references:
            ...
            for idx, d in enumerate(self.get("references"), start=1):
                ...
                if (
                    d.payment_term and flt(d.allocated_amount) > 0 and latest.payment_term_outstanding and (flt(d.allocated_amount) > flt(latest.payment_term_outstanding)) and self.term_based_allocation_enabled_for_reference(d.reference_doctype, d.reference_name)
                ):
                    frappe.throw(
                        _(
    ...
    def add_advance_gl_entries(self, gl_entries: list, entry: object | dict | None):
        ...
        """
        if self.book_advance_payments_in_separate_party_account:
            references = list(self.get("references"))
            if entry:
    ...
    if args.get("party_type") == "Supplier":
        ...
        if supplier_status["on_hold"]:
            if supplier_status["hold_type"] == "All":
                return []
            if supplier_status["hold_type"] == "Payments":
                if (
...
def get_split_invoice_rows(invoice: dict, payment_term_template: str, exc_rates: dict) -> list:
    ...
    for payment_term in payment_schedule:
        ...
        is_multi_currency_acc = doc_details.company_currency not in (doc_details.currency, doc_details.party_account_currency)
...
def get_payment_entry(
    ...
    if doc.doctype == "Purchase Invoice" and doc.invoice_is_blocked():
        ...
        if doc.doctype in {"Sales Invoice", "Purchase Invoice", "Purchase Order", "Sales Order"} and frappe.get_cached_value(
...
def set_payment_type(dt, doc):
    if (
        dt == "Sales Order" or dt == "Dunning" or (dt == "Sales Invoice" and doc.outstanding_amount > 0) or (dt == "Purchase Invoice" and doc.outstanding_amount < 0)
    ):
...
def get_reference_as_per_payment_terms(
    ...
    references = []
    is_multi_currency_acc = doc.company_currency not in (doc.currency, party_account_currency)
