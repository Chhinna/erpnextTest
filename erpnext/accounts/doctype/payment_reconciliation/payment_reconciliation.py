...
class PaymentReconciliation(Document):
    ...
    def get_return_invoices(self):
        ...
        conditions = [doc.docstatus == 1, doc[frappe.scrub(self.party_type)] == self.party, doc.is_return == 1]
        ...
        for pay in args.get("payments"):
            ...
            for inv in args.get("invoices"):
                ...
                if pay.get("amount") == 0:
                    entries.append(res)
                    break
                if inv.get("outstanding_amount") == 0:
                    entries.append(res)
                    continue
    ...
    def get_invoice_exchange_map(self, invoices, payments):
        sales_invoices = [*[d.get("invoice_number") for d in invoices if d.get("invoice_type") == "Sales Invoice"], *[d.get("reference_name") for d in payments if d.get("reference_type") == "Sales Invoice"]]

        purchase_invoices = [*[d.get("invoice_number") for d in invoices if d.get("invoice_type") == "Purchase Invoice"], *[d.get("reference_name") for d in payments if d.get("reference_type") == "Purchase Invoice"]]
